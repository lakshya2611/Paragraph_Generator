...
form.addEventListener("submit", function (e) {
    e.preventDefault();
    // para.innerHTML = `<p>${demo[0]}</p>`;

    let inputvalue = parseInt(input.value);
    console.log(inputvalue);

    if( inputvalue == 0 || isNaN(inputvalue)  || inputvalue>10){
        para.style.fontSize = "20px";
        para.innerHTML = `<p class="para">${demo[0]}</p>`;
    }
    else {
        // console.log(20);
        inputvalue = parseInt(input.value);
        let newarr = demo.slice(0, inputvalue);
        console.log(newarr);
        newarr = newarr.map(function (item) {
            return `<p class="para">${item}</p>`;
        }).join("");

        para.innerHTML = newarr;
    }
});